...
class ProductCategoryController extends Controller
{
    ...
    public function getSIMSuggestions(Request $request)
    {
        ...
        foreach ((new Products)->getSIMSuggetions($request->all()) as $product) {
            $data[] = [
                'id' => $product->id,
                'name' => $product->lang1_name,
            ];
        }
    ...
    public function getProductSuggestions(Request $request)
    {
        ...
        foreach ((new Products)->getProductSuggetions($request->all()) as $product) {
            $data[] = [
                'id' => $product->id,
                'name' => $product->code . ' (' . $product->lang1_name . ')',
            ];
        }
        return response()->json($data, 200);
    }
