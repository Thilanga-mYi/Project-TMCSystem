...
class CustomerController extends Controller
{
    ...
    public function customerList()
    {
        ...
        foreach ($records as $key => $customer) {
            ...
            $tableData[] = [
                ...
                $actions,
                $customer->name,
                ($customer->nic_or_passport != '' ? $customer->nic_or_passport : '-'),
    ...
    public function save(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'new_customer_email' => 'required|email',
            'new_customer_nic' => 'required_if:new_customer_registration_number,null',
            'new_customer_registration_number' => 'required_if:new_customer_nic,null',
        ...
        if (!empty($request->new_customer_password_send)) {
            $api_request = (new Api)->addUser($email);
        }
        ...
        if ($api_request != 'success') {
            return response()->json([
                'code' => 101,
                'type' => 'error',
                'des' => 'Something went wrong: ' . $api_request,
                'refresh_status' => 2,
        ...
        $data = [
            ...
            'nic_or_passport' => $request->new_customer_nic,
            ...
            'email' => $request->new_customer_email,
            'remark' => $request->new_customer_bank_details,
    ...
    public function updateCustomer(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'update_customer_nic' => 'required_if:update_customer_registration_number,null',
            'update_customer_registration_number' => 'required_if:update_customer_nic,null',
            'update_customer_city' => 'required',
            'update_customer_street_address' => 'required',
            'update_customer_tel1' => 'required|numeric|digits:10|min:0',
        ]);
        ...
        $data = [
            'company_name' => $request->update_customer_company_name,
            'nic_or_passport' => $request->update_customer_nic,
            'company_regis' => $request->update_customer_registration_number,
            'street_address' => $request->update_customer_street_address,
            'city' => $request->update_customer_city,
            'tel1' => $request->update_customer_tel1,
            'tel2' => $request->update_customer_tel2,
            'email' => $request->update_customer_email,
            'bank_details' => $request->update_customer_bank_details,
            'status' => 1,
        ];
    ...
    public function getCustomerPrimaryDetails(Request $request)
    {
        return (new Customer)->where('id', $request->id)->first();
    }
}
